...
import { addCategory, completeLine, toggleSidebar, setinput } from "../app/appSlice";
import Suggestion from "./Suggestion";

const Header = () => {
    const serchSeggestionApi = import.meta.env.VITE_SEARCH_SUGGESTIONS_API;
    const input = useSelector((store)=>store.app.input)
    ...
    const showSuggestion = async () => {
        try {
            const res = await fetch(`${serchSeggestionApi}${input}`);
            const data = await res.json();
            console.log(data[1]);
            dispatch(completeLine(data[1]))
        } catch (error) {
            console.log(error)
        }
    }
    ...
    return (
        <>
            <nav className="flex justify-between items-center px-4 py-2 bg-white shadow fixed top-0 left-0 right-0 z-10">
                <div className="left flex gap-4 items-center">
                    <div onClick={() => dispatch(toggleSidebar())} className="hamburger cursor-pointer"><GiHamburgerMenu /></div>
                    <div className="logo w-28 cursor-pointer">
                        <svg
                ...
                <div className="search flex items-center w-1/2">
                    <input value={input} onChange={(e) => { dispatch(setinput(e.target.value)) }}
                        className="border px-4 py-1 w-full rounded-s-xl"
                        type="text"
                        placeholder="Search"
                        name=""
                        id=""
                    />
                    <button onClick={serchVideo} className="srch-btn px-2 py-2  border rounded-e-xl">
                        <IoIosSearch />
                    </button>
                    <div className="mic ml-4 cursor-pointer rounded-full p-3">
                        <MdOutlineMicNone />
                    </div>
                </div>
                <div className="right flex gap-2 items-center">
                    <div className="bell cursor-pointer  rounded-full p-3">
                        <GoBell />
                    </div>
                    <div className="profile cursor-pointer ">
                        <img
                            className="rounded-full p-3 w-12"
                            src="https://pxbar.com/wp-content/uploads/2023/10/natural-attractive-whatsapp-dp.jpg"
                            alt="user"
                        />
                    </div>
                </div>
            </nav>
            {input !== "" && <div className="fixed left-[415px] w-[43%] top-[3.2rem] z-40">
            <Suggestion/>
            </div>}
        </>
    );
